...
self.addEventListener('fetch', e => {
  ...
  e.respondWith(fetch(e.request).then(res=>{
    if(res&&res.status===200&&e.request.method==='GET'){caches.open(CACHE).then(c=>c.put(e.request,res.clone()));}
    return res;
...
self.addEventListener('push', e => {
  ...
  e.waitUntil(self.registration.showNotification(data.title||'💑 Notre Agenda',{
    body: data.body||'Nouvel événement',
...
// Quand le SW se réveille (sync background), il demande aux clients actifs
// de relancer les vérifications de rappels
...
function notifyClients(){
  return clients.matchAll({type:'window',includeUncontrolled:true}).then(cls=>{
    cls.forEach(c=>c.postMessage({type:'CHECK_REMINDERS'}));
  });
}
